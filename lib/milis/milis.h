...
#define DELAY_MS_MAX_VALUE (0xffff-2) // don't know if the maximum value can be 0xFFFF, for that reason, I am setting it to a lower value.
uint16_t milis(void);
void delay_ms(uint16_t del);

#elif defined(MILIS_32BIT)

#define DELAY_MS_MAX_VALUE (0xffffffff-2) // don't know if the maximum value can be 0xFFFF, for that reason, I am setting it to a lower value.
uint32_t milis(void);
